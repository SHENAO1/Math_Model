             ...
             if all((temp_m(2:4)-temp_m(1:3))>1)
               ...
               for j=1:3
                ...
                if abs(sum(temp))>0.00001
                  Len1(i)=sum([D(path(i,1:m-1)+m*(path(i,2:m)-1)) D(path(i,m)+m*(path(i,1)-1))]);%*((65-temp)/65*2);
                else
                  Len1(i)=9999;
                end
             else
          ...
          for i=1:pn
             ...
             if all((temp_m(2:4)-temp_m(1:3))>1)
               ...
               for j=1:3
                ...
                if abs(sum(temp))>0.00001
                  Len2(i)=sum([D(path2(i,1:m-1)+m*(path2(i,2:m)-1)) D(path2(i,m)+m*(path2(i,1)-1))]);%*((65-temp)/65*2);
                  %一定要注意
                else
                  Len2(i)=9999;
                end
