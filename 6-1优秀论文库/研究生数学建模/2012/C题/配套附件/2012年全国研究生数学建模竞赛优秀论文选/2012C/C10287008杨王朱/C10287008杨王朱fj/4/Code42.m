...
%计算一级杆
DR1=25/1000.0;    %一级杆杆径(m)
LL1=523.61;       %一级杆杆长(m)
DR2=22/1000.0;    %二级杆杆径(m)
LL2=664.32;       %二级杆杆长(m)
DR3=19/1000.0;    %三级杆杆径(m)
LL3=618.35;       %三级杆杆长(m)
rul=864*(1-0.912)+1000*0.912; 
%TM=(ru-864)*((pi*DR1*DR1/4)*LL1)*9.8
%D=D-TM; %去重力载荷
%计算sig0,vel0
sig0=0.0;
vel0=0.0;
...
for p=1:k
   Xx(p)=p/k*360;
   Ux(p)=tp0*LL+vel0/2-(ru-rul)*9.8/ru/2/AA^2*LL^2;
   Fx(p)=sig0/2-EA*(ru-rul)*9.8/ru/AA^2*LL;
   for i=1:n
...
%计算二级杆
U=Ux;
%TM=(ru-864)*((pi*DR2*DR2/4)*LL2)*9.8;
%D=Fx-TM; %去重力载荷
D=Fx;
%计算sig0,vel0
sig0=0.0;
...
%计算三级杆
U=Ux;
%TM=(ru-864)*((pi*DR3*DR3/4)*LL3)*9.8;
%D=Fx-TM; %去重力载荷
D=Fx;
%计算sig0,vel0
sig0=0.0;
...
%减油压，套压差
Fp=(0.3-0.2)*pi/4*44^2;
Fx=Fx-Fp;

plot(Ux,Fx,'r-')
hold on
fid = fopen('case423.txt', 'wt');
for i=1:k
    fprintf(fid, '%f\t%f\n', Ux(i),Fx(i));
end 
fclose(fid);