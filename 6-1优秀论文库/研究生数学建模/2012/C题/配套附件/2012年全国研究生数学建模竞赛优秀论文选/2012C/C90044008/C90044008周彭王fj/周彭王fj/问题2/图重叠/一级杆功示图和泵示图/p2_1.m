...
plot(weiyi(i),huatu(i),'kx')
 hold on
 plot(u(i),F(i)/10^3,'k.')
 h1 = xlabel('位移(m)'); h2 = ylabel('载荷(kN)'); 
 h=legend('示功图','泵功图',1);
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
set(h,'box','off');%去掉边框
set(gca,'xtick',0:1:3)
set(gca,'ytick',-20:20:70)
set(gca,'fontsize',9);           % 坐标轴上的数字字号为9 pt
set(h1,'fontsize',9);             %  X轴的字号为9 pt
set(h2,'fontsize',9);             %  Y轴的字号为9 pt
print('-djpeg','-r600','示功图和泵功图')          % 以600 dpi的分辨率输出一个JPG 图片
print('-dtiff','-r300','示功图和泵功图')  % 以300 dpi的分辨率输出一个tiff 的 图片
saveas(gcf,'示功图和泵功图.fig')% 输出fig

u;
F=F/1000;
result=[u;F];
xlswrite('一级杆位移和载荷.xls',result)
save weiyizaihe u F
