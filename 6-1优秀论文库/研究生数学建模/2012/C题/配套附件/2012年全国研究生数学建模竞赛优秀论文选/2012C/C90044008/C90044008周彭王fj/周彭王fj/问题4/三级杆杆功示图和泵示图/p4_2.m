...
for i=1:N-1
    ...
    tao2(1)=0;
    delta2(1)=0;
    new=[];new_diff=[];sigma=[];tao=[];v=[];delta=[];alpha=[];beta=[];x=[];miu=[];O=[];P=[];O_diff=[];P_diff=[];
    sigma=sigma2;tao=tao2;v=v2;delta=delta2;h=h2;fr=fr2;
    new=-g/2/a^2*h^2-g/2/a^2*h2^2;new_diff=-E*fr*g*h/a^2-E*fr*g*h2/a^2;
  for i=1:N-1
    alpha(i)=i*omiga/(a*sqrt(2))*sqrt(1+sqrt(1+(c/(i*omiga))^2));
    beta(i)=i*omiga/(a*sqrt(2))*sqrt(-1+sqrt(1+(c/(i*omiga))^2));
    x(i)=(sigma(i+1)*alpha(i)+tao(i+1)*beta(i))/(E*fr*(alpha(i)^2+beta(i)^2));
...
for j=1:k
    qiuhe1=O(1:N-1).*cos((1:N-1).*omiga*t(j))+P(1:N-1).*sin((1:N-1)*omiga*t(j));  %位移函数中的求和部分
    qiuhe2=O_diff(1:N-1).*cos((1:N-1).*omiga*t(j))+P_diff(1:N-1).*sin((1:N-1)*omiga*t(j));  %载荷函数中的求和部分
    uyinzi2(j)=sigma(1)/(2*E*fr)*h+v(1)/2+sum(qiuhe1)+new;
    Fyinzi2(j)=E*fr*(sigma(1)/(2*E*fr)+sum(qiuhe2))+new_diff;%柱塞的载荷计算公式
end
...
for i=1:N-1
    ...
    delta3(1)=0;
    new=[];new_diff=[];sigma=[];tao=[];v=[];delta=[];alpha=[];beta=[];x=[];miu=[];O=[];P=[];O_diff=[];P_diff=[];
    sigma=sigma3;tao=tao3;v=v3;delta=delta3;
    h=h3;
    fr=fr3;
    new=-g/2/a^2*h^2-g/2/a^2*h2^2-g/2/a^2*h3^2;
    new_diff=-E*fr*g*h/a^2-E*fr2*g*h2/a^2-E*fr3*g*h3/a^2;
    for i=1:N-1
    alpha(i)=i*omiga/(a*sqrt(2))*sqrt(1+sqrt(1+(c/(i*omiga))^2));
    beta(i)=i*omiga/(a*sqrt(2))*sqrt(-1+sqrt(1+(c/(i*omiga))^2));
    x(i)=(sigma(i+1)*alpha(i)+tao(i+1)*beta(i))/(E*fr*(alpha(i)^2+beta(i)^2));
...
for j=1:k
    qiuhe1=O(1:N-1).*cos((1:N-1).*omiga*t(j))+P(1:N-1).*sin((1:N-1)*omiga*t(j));  %位移函数中的求和部分
    qiuhe2=O_diff(1:N-1).*cos((1:N-1).*omiga*t(j))+P_diff(1:N-1).*sin((1:N-1)*omiga*t(j));  %载荷函数中的求和部分
    uyinzi3(j)=sigma(1)/(2*E*fr)*h+v(1)/2+sum(qiuhe1)+new;
    Fyinzi3(j)=E*fr*(sigma(1)/(2*E*fr)+sum(qiuhe2))+new_diff;%柱塞的载荷计算公式
end
...
hold on
plot(uyinzi3(cc),Fyinzi3(cc)/10^3,'b-v')
h=legend('未加因子','加因子',1);
set(h,'box','off');%去掉边框
axis([-0.5 7 -20 45])
