...
load xuangongtu
% % 归一化
s=2*(s-min(s))/(max(s)-min(s));%位移
f=(f-min(f))/(max(f)-min(f))-0.5;%载荷
l=length(s);
G=zeros(m-1,n-1);
for k=1:l
    for i=1:m-1
...
print('-djpeg','-r600','附件2灰度图')          % 以600 dpi的分辨率输出一个JPG 图片
print('-dtiff','-r300',' 附件2灰度图')  % 以300 dpi的分辨率输出一个tiff 的 图片
saveas(gcf,' 附件2灰度图.fig')% 输出fig