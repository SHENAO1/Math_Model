...
poss1x=zeros(1,N);%%%%第一个卫星坐标系下的横坐标
poss2x=zeros(1,N);
ran=[];
t=50;
for t=49:171
[tempp1,tempp2,temp1,temp2]=crosslocal(y6,y9,t);
pos1(:,t+1)=tempp1;
pos2(:,t+1)=tempp2;
poss1x(t+1)=temp1;
poss2x(t+1)=temp2;
...
for t=50:4:130
    jieini=[pos1(:,t+1)'  poss1x(t+1) poss2x(t+1)  pos1(:,t+2)'  poss1x(t+2) poss2x(t+2) pos1(:,t+3)'  poss1x(t+3) poss2x(t+3) pos1(:,t+4)'  poss1x(t+4) poss2x(t+4) 1e-5 1e-5 1e-5 1e-5]';
    jie=fsolve(@(x)myfun(x,y6,y9,t),jieini,options);
    delta=[delta jie(21:24)];
end
%%%
delta=mean(delta');%%%%%%%%系统误差
...
t=50;
for t=49:171
[tempp1,tempp2]=mod_crosslocal(y6,y9,delta,t);
pos1(:,t+1)=tempp1;
pos2(:,t+1)=tempp2;
poss1x(t+1)=temp1;
poss2x(t+1)=temp2;
...
%------------位置估计残差----------------%
res1_mod=pos1(:,51:end);
res2_mod=y_res(1:2:5,50:170);
for i=1:length(res2_mod)
    error1_mod(:,i)=res1_mod(:,i)-res2_mod(:,i);
end
disp('t=50:10:170');
error1_mod(:,1:10:121)
%--------------------------------------%
%------------速度估计残差----------------%
res3_mod=[xx(2,51:end);yy(2,51:end);zz(2,51:end)];
res4_mod=y_res(2:2:6,50:170);
for i=1:length(res4_mod)
    error2_mod(:,i)=res3_mod(:,i)-res4_mod(:,i);
end
disp('t=50:10:170');
error2_mod(:,1:10:121)
...
% t=50;
% for t=49:171
% [tempp1,tempp2]=crosslocal(y6,y9,delta,t);
% pos1(:,t+1)=tempp1;
% pos2(:,t+1)=tempp2;
% poss1x(t+1)=temp1;
% poss2x(t+1)=temp2;
