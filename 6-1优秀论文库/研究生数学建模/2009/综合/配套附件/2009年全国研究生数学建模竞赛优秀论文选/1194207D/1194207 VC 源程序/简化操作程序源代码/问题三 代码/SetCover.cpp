...
CSetCover::~CSetCover(void)
{
	...
	if(m_pathArray!=NULL)
	{
		for(int i=0;i<m_existCars;i++)
			m_pathArray[i].RemoveAll();
		delete [] m_pathArray;
		m_pathArray=NULL;
	}
...
void CSetCover::SCHF()
{
	...
	*/
	...
	CString ms;
	ms.Format("当前最优解：分配 %d 辆警车",m_cover.GetCount());
	AfxMessageBox(ms);
...
void CSetCover::ShowPoliceCarMove(int icar, int tim)
{
	...
	while(searchLength>0.0) //搜索长度为0，表示搜索完毕
	{
		...
		for(int j=0;j<m_linkList[curPos].GetCount();j++)
		{
			if(j==0)
			{
				...
				nextPos = m_linkList[curPos].GetAt(j);
			}
			else
			{
	...
	*/
	...
	for(int i=0;i<m_weight[m_cover.GetAt(icar)];i++)
		m_linkList[i].RemoveAll();
	m_linkList = NULL;
