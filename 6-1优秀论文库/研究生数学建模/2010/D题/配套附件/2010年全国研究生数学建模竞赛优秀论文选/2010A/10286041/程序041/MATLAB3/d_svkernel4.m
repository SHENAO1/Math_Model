function [d_k] = d_svkernel4(M,ker,u,v,i,j)
...
%      du=(2*randn(size(u))-1)*1;
%     dv=(2*randn(size(v))-1)*1;
     du=M_mean-M_mean1;
   %  du=-du*(6.5+(2*randn(1,1)-1)*0);
   du=-du*0.01;
     dv=M_mean-M_mean2;
     dv=-dv*0.01;
  %   dv=-dv*(6.5+(2*randn(1,1)-1)*0);
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     %噪声放大因子在6附近时最好
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% d_k=sum((u+v)*0.01)
      case 'poly'
        k = (u*v' + 1)^p1;
      case 'rbf'
        k = exp(-(u-v)*(u-v)'/(2*pi^2));
        d_k=k/(2*p1)/2/pi*2*(u-v).*(du-dv);
      case 'erbf'
