...
for cc=1:cluster
    ...
    for isf=1:csf
        if sefe(isf)==1
            newtrainf=[newtrainf feature(:,isf)];
            newtestf=[newtestf testfeature(:,isf)];
            newcenter=[newcenter center(:,isf)];
        end
    end
%KNN 分类
K=3;
...
for i=1:r
   for j=1:rtrain
       d=sp_dist2(newtestf(i,:),newtrainf(j,:));
       dis(j)=d;
   end
 ...
 if bb==cc
    aaaa=sp_dist2(newcenter(cc,:),newtestf(i,:));
    if aaaa<=maxerror(bb)
        resultlabel(i)=bb;
    else
        resultlabel(i)=bb;  % 用-决定异类
    end
    end
