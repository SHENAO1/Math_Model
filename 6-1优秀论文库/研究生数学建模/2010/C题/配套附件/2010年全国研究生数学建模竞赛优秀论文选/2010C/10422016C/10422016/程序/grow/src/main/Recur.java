...
import swcparts.SwcLine;
import swcparts.SwcNeuron;

public class Recur {
	public static int TIMES=1000; //recursive times...
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		...
		int vari=0;
		DistillSwc.OUTPUT_DIR="tes";
		while(vari<TIMES){

			DistillSwc.INPUT_FILE="tes/"+vari+".swc";
	        ...
	        PrintWriter writer;
			try{
				writer=new PrintWriter(new FileWriter(DistillSwc.OUTPUT_DIR+"\\"+vari+".swc"));
				neuron.writeSWC(writer);
				writer.close();
			}catch(Exception e){
				System.out.println(e.getLocalizedMessage());
